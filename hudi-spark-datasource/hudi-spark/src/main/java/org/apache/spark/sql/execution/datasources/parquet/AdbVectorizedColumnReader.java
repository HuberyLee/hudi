...
import static org.apache.spark.sql.execution.datasources.parquet.AdbSpecificParquetRecordReaderBase.createAdbRLEIterator;

/**
 * Decoder to return values from a single column.
 */
public class AdbVectorizedColumnReader {
  /**
   ...
   */
  private AdbSpecificParquetRecordReaderBase.IntIterator repetitionLevelColumn;
  private AdbSpecificParquetRecordReaderBase.IntIterator definitionLevelColumn;
  private ValuesReader dataColumn;

  // Only set if vectorized decoding is true. This is used instead of the row by row decoding
  // with `definitionLevelColumn`.
  private AdbVectorizedRleValuesReader defColumn;
  ...
  public AdbVectorizedColumnReader(
    ...
    while (total > 0) {
      ...
      if (isCurrentPageDictionaryEncoded) {
        ...
        defColumn.readIntegers(
            num, dictionaryIds, column, rowId, maxDefLevel, (AdbVectorizedValuesReader) dataColumn);
  ...
  private void readBooleanBatch(int rowId, int num, WritableColumnVector column)
    ...
    defColumn.readBooleans(
        num, column, rowId, maxDefLevel, (AdbVectorizedValuesReader) dataColumn);
  }
  ...
  private void readBinaryBatch(int rowId, int num, WritableColumnVector column) throws IOException {
    // This is where we implement support for the valid type conversions.
    // TODO: implement remaining type conversions
    AdbVectorizedValuesReader data = (AdbVectorizedValuesReader) dataColumn;
  ...
  private void initDataReader(Encoding dataEncoding, ByteBufferInputStream in) throws IOException {
    ...
    if (dataEncoding.usesDictionary()) {
      ...
      this.dataColumn = new AdbVectorizedRleValuesReader();
      this.isCurrentPageDictionaryEncoded = true;
    } else {
      if (dataEncoding != Encoding.PLAIN) {
        throw new UnsupportedOperationException("Unsupported encoding: " + dataEncoding);
      }
      this.dataColumn = new AdbVectorizedPlainValuesReader();
      this.isCurrentPageDictionaryEncoded = false;
    }
  ...
  private void readPageV1(DataPageV1 page) throws IOException {
    ...
    this.defColumn = new AdbVectorizedRleValuesReader(bitWidth);
    dlReader = this.defColumn;
    this.repetitionLevelColumn = new AdbSpecificParquetRecordReaderBase.AdbValuesReaderIntIterator(rlReader);
    this.definitionLevelColumn = new AdbSpecificParquetRecordReaderBase.AdbValuesReaderIntIterator(dlReader);
    try {
  ...
  private void readPageV2(DataPageV2 page) throws IOException {
    this.pageValueCount = page.getValueCount();
    this.repetitionLevelColumn = createAdbRLEIterator(descriptor.getMaxRepetitionLevel(),
        page.getRepetitionLevels(), descriptor);

    int bitWidth = BytesUtils.getWidthFromMaxInt(descriptor.getMaxDefinitionLevel());
    // do not read the length from the stream. v2 pages handle dividing the page bytes.
    this.defColumn = new AdbVectorizedRleValuesReader(bitWidth, false);
    this.definitionLevelColumn = new AdbSpecificParquetRecordReaderBase.AdbValuesReaderIntIterator(this.defColumn);
