...
package org.apache.spark.sql.execution.datasources.parquet.util

import org.apache.spark.sql.execution.datasources.parquet.util.DateTimeConstants._
import org.apache.spark.sql.execution.datasources.parquet.util.RebaseDateTime._
...
object DateTimeUtils {
  ...
  def dateAddInterval(
    ...
    val ld = daysToLocalDate(start).plusMonths(interval.months).plusDays(interval.milliseconds() / 24 / 3600 / 1000)
    localDateToDays(ld)
  }
  ...
  def subtractDates(endDay: Int, startDay: Int): CalendarInterval = {
    ...
    new CalendarInterval(months, days * 24 * 3600 * 1000)
  }
}
