...
import org.apache.spark.sql.execution.AdbConstant;
import org.apache.spark.sql.execution.datasources.GeneratePredicateAdapter;
import org.apache.spark.sql.execution.vectorized.ColumnVectorAllocator;
 ...
 */
public class AdbVectorizedParquetRecordReader extends AdbSpecificParquetRecordReaderBase<Object> {
   ...
   */
  private AdbVectorizedColumnReader[] columnReaders;
  ...
  private Predicate prewherePredicate = null;
  private int[] validRowsId = null;

  public AdbVectorizedParquetRecordReader(TimeZone convertTz, boolean useOffHeap, int capacity) {
  ...
  public void initialize(InputSplit inputSplit, TaskAttemptContext taskAttemptContext)
    ...
    Seq<Expression> prewhereFilters = SerializationUtil.readObjectFromConfAsBase64(
        AdbConstant.ADB_PREWHERE_PREDICATE, hadoopConf);
    Seq<Attribute> output = SerializationUtil.readObjectFromConfAsBase64(
        AdbConstant.ADB_PREWHERE_OUTPUT, hadoopConf);

    prewherePredicate = new GeneratePredicateAdapter(prewhereFilters, output).generate();
    prewherePredicate.initialize(0);
  }
  ...
  public Object getCurrentValue() {
    ...
    return columnarBatch.getRow(validRowsId[batchIdx - 1]);
  }
  ...
  private void initBatch(
    ...
    columnVectors = ColumnVectorAllocator.allocateColumns(memMode, capacity, batchSchema);
  ...
  public boolean nextBatch() throws IOException {
    ...
    return exePrewherePredicate();
  }

  private boolean exePrewherePredicate() {
    if (validRowsId == null || validRowsId.length < columnarBatch.numRows()) {
      validRowsId = new int[columnarBatch.numRows()];
    }

    int index = 0;
    for (int rowId = 0; rowId < columnarBatch.numRows(); rowId++) {
      if (prewherePredicate.eval(columnarBatch.getRow(rowId))) {
        validRowsId[index++] = rowId;
      }
    }
  ...
  private void checkEndOfRowGroup() throws IOException {
    ...
    columnReaders = new AdbVectorizedColumnReader[columns.size()];
    for (int i = 0; i < columns.size(); ++i) {
      if (missingColumns[i]) continue;
      columnReaders[i] = new AdbVectorizedColumnReader(columns.get(i), types.get(i).getOriginalType(),
