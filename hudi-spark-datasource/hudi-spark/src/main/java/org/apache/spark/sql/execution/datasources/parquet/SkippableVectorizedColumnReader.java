...
import org.apache.spark.sql.execution.datasources.parquet.util.DateTimeUtils;
import org.apache.spark.sql.execution.datasources.parquet.util.RebaseDateTime;
    ...
    static int rebaseDays(int julianDays, final boolean failIfRebase) {
        if (failIfRebase) {
            if (julianDays < RebaseDateTime.lastSwitchJulianDay()) {
                throw new RuntimeException("Rebase exception in read");
            } else {
                return julianDays;
            }
        } else {
